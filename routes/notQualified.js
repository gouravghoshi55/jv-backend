...
// GET /api/not-qualified/list
router.get("/list", async (req, res) => {
  try {
    const data = await getSheetData(SHEETS.NOT_QUALIFIED);
    if (data.length <= 1) {
    ...
    console.error("Not Qualified list error:", err);
    res.status(500).json({ error: "Failed to fetch", details: err.message });
  }
});

// POST /api/not-qualified/update
router.post("/update", async (req, res) => {
  try {
    ...
    const rowIndex = await findRowByEnqNo(SHEETS.NOT_QUALIFIED, enqNo);
    if (rowIndex === -1) {
    ...
    if (destinationSheet !== SHEETS.FMS) {
      moveRow.push("");            // Status (empty in destination)
      moveRow.push(remarks || ""); // Remarks
    }
    

    await appendRow(destinationSheet, moveRow);
    await deleteRow(SHEETS.NOT_QUALIFIED, rowIndex);
    ...
    console.error("Not Qualified update error:", err);
